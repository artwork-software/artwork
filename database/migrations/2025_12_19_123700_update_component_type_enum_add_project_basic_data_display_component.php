...
return new class extends Migration
{
    ...
    {
        // Diese Migration ist nur relevant, solange `components.type` ein ENUM ist.
        // Falls die Spalte bereits auf STRING umgestellt wurde (z.B. durch spätere Migrationen),
        // überspringen wir die Änderung, um kein Downgrade zurück auf ENUM zu verursachen.
        if (!Schema::hasColumn('components', 'type')) {
            return;
        }

        $column = DB::selectOne("SHOW COLUMNS FROM components LIKE 'type'");
        ...
        Schema::table('components', function (Blueprint $table) {
            $table->enum('type_temp', [
                ...
                'ArtistNameDisplayComponent','ProjectBasicDataDisplayComponent'
            ])->nullable()->after('type');
        });
    ...
    {
        // Nur relevant, wenn `components.type` ein ENUM ist.
        if (!Schema::hasColumn('components', 'type')) {
            return;
        }
