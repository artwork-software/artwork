        ...
        <div v-for="day in days">
            ...
            <div class="flex items-center sticky gap-0.5 h-16 bg-gray-200 z-20 first-line:divide-none top-[128px]">
                <div class="flex xxsDark items-center px-8 mt-1.5" :style="{marginLeft: zoom_factor === 0.2 ? '50px' : zoom_factor * 90 + 'px'}">
                    {{ daysWithoutEventsToDisplayHiddenHours.includes(day.full_day)
            ...
            <div v-for="hour in day.hours_of_day">
                <div v-if="shouldShowHour(hour, calendarData, day)" class="border-b border-dashed">
                    <div class="w-fit events-by-days-container rounded-lg" ref="calendarToCalculate">
                        ...
                        : {}">
                                        ...
                                        <div v-for="(event, index) in (room.content[day.full_day]?.events || [])" :key="event.id">
...
const shouldShowHour = (hour, calendarData, day) => {
    ...
    const hasEventsToday = calendarData.some(room =>
        (room.content[day.full_day]?.events || []).length > 0
    );
    ...
    if (!hasEventsToday) {
        ...
        if (!daysWithoutEventsToDisplayHiddenHours.value.includes(day.full_day)) {
            daysWithoutEventsToDisplayHiddenHours.value.push(day.full_day);
        }
    ...
    const index = daysWithoutEventsToDisplayHiddenHours.value.indexOf(day.full_day);
    if (index !== -1) {
