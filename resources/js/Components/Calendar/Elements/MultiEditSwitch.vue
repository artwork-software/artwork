<template>
    <Switch v-if="!roomMode" v-model="enableMultiEdit" @update:model-value="$emit('update:multiEdit', enableMultiEdit)" :class="[enableMultiEdit ? 'bg-artwork-buttons-hover' : 'bg-gray-200', 'relative inline-flex items-center h-5 w-10 flex-shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none focus:ring-none']">
        <span :class="[enableMultiEdit ? 'translate-x-5' : 'translate-x-0', 'relative inline-block h-6 w-6 border border-gray-300 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out']">
            <span :class="[enableMultiEdit ? 'opacity-0 duration-100 ease-out' : 'opacity-100 duration-200 ease-in z-40', 'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity']" aria-hidden="true">
                <ToolTipComponent
                    direction="bottom"
            ...
            <span :class="[enableMultiEdit ? 'opacity-100 duration-200 ease-in z-40' : 'opacity-0 duration-100 ease-out', 'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity']" aria-hidden="true">
                <ToolTipComponent
                    direction="bottom"
                    :tooltip-text="$t('Edit')"
                    icon="IconPencil"
