        ...
        <div v-if="isChatOpen">
            <div class="shadow-lg bg-white rounded-lg w-96">
                <div class="bg-artwork-buttons-default px-4 py-4 rounded-t-lg">
                    <div class="flex items-center justify-between">
                        <div class="font-lexend text-white text-xs font-bold">
                            Artwork Chat <span v-if="chatPartner.length !== 0">mit {{ chatPartner.name }}</span>
                        </div>
                ...
                <div class="bg-white px-3 py-2">
                    ...
                    <div v-else>
                        <div class="font-lexend text-xs font-medium flex items-center cursor-pointer" @click="chatPartner = []">
                ...
                <div v-else>
                    <div class="bg-white rounded-b-lg max-h-96 min-h-96 overflow-y-auto">
                        <div class="space-y-3" v-if="chatPartner.length === 0">
                            <div class="px-3 py-2 hover:bg-blue-50 transition duration-200 ease-in-out" v-for="chat in chats">
                                <div @click="openChatPage(chat.id)">
                    ...
                    <div v-if="chatPartner.length !== 0" class="bg-white rounded-b-lg px-3 py-4">
...
const openChatPage = async(chatId) => {
    ...
    // Hier kannst du die Logik hinzufügen, um den Chat zu öffnen
    // und die Nachrichten anzuzeigen. Zum Beispiel:
    try {
        const response = await axios.get(route('chat-system.get-chat-messages', { chat: chatId }))
        chatPartner.value = response.data.chat
        isLoading.value = false
        // Hier kannst du die Nachrichten in deinem Chat anzeigen
    } catch (error) {
