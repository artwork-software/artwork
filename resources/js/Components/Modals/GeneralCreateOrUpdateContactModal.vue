    ...
    <ArtworkBaseModal @close="$emit('close')" :title="title" :description="description">
        <form @submit.prevent="createOrUpdateContact" class="space-y-6">
            ...
            <div class="flex items-center justify-between mt-5">
                <BaseUIButton is-add-button type="submit" :label="props.contact.id ? $t('Update') : $t('Create')" />
                <BaseUIButton is-cancel-button :label="$t('Cancel')" type="button" @click="$emit('close')" />
            </div>
        </form>
    </ArtworkBaseModal>
</template>
