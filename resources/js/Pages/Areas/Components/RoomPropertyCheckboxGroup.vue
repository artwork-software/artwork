<template>
    <div>
        <h3 class="text-sm font-semibold mb-3" :class="darkText ? 'text-text' : 'text-white'">{{ label }}</h3>
        <div v-if="items?.length > 0" class="grid gap-x-4 gap-y-2" :class="singleColumn ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'">
            <label v-for="item in items"
                ...
                <span class="flex items-baseline gap-2 text-sm truncate" :class="darkText ? 'text-text' : 'text-white'">
                    <span class="truncate">{{ item.name }}</span>
                    <span v-if="item.hint" class="text-xs shrink-0" :class="darkText ? 'text-text-subtle' : 'text-text-subtle'">{{ item.hint }}</span>
                </span>
            </label>
        </div>
        <p v-else class="text-xs" :class="darkText ? 'text-text-subtle' : 'text-text-subtle'">{{ emptyText }}</p>
    </div>
</template>
