<template>
    ...
    <div class="drag-item w-48 p-2 bg-surface-sunken/10 text-white text-xs rounded-lg flex items-center gap-2" draggable="true" @dragstart="onDragStart">
        <div class="cursor-pointer w-full">
            <div class="w-full flex items-center justify-between h-8">
                <div>
                    {{ item?.name }}
                </div>
                <div class="text-[9px] bg-surface-sunken/10 rounded-full px-2 py-1">
                    {{ item?.count }}
                </div>
            </div>
        </div>
