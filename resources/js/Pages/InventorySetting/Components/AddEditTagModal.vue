<template>
    ...
    >
        <form @submit.prevent="submit" class="space-y-8">
            <!-- SECTION: Basics -->
            <div class="rounded-2xl border border-border-subtle bg-gradient-to-br from-white via-surface-sunken to-surface-sunken p-5 space-y-5 shadow-sm">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-sm font-semibold text-text flex items-center gap-2">
                            <span
                                class="inline-flex h-6 w-6 items-center justify-center rounded-xl bg-accent-100 text-accent-600 text-xs font-bold"
                            >
                                1
                            </span>
                            {{ $t('Tag basics') }}
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <ArtworkBaseListbox
                                ...
                                :clearable="true"
                            />
                            <p v-if="form.errors.inventory_tag_group_id" class="mt-1 text-xs text-danger">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col items-center gap-1">
                                ...
                                <span
                                    class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium border border-border-subtle"
                        ...
                        <p v-if="form.errors.color" class="mt-1 text-xs text-danger">{{ form.errors.color }}</p>
                        <p class="mt-1 text-xs text-text-subtle">
            ...
            <!-- SECTION: Permissions -->
            <div class="rounded-2xl border border-border-subtle bg-gradient-to-br from-white via-surface-sunken to-surface-sunken p-5 space-y-6 shadow-sm">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-sm font-semibold text-text flex items-center gap-2">
                            <span
                                class="inline-flex h-6 w-6 items-center justify-center rounded-xl bg-success-surface text-success text-xs font-bold"
                            >
                                2
                            </span>
                            {{ $t('Permissions') }}
                ...
                >
                    ...
                    <div class="relative">
                        ...
                        <div
                            ...
                            class="absolute z-20 mt-1 w-full max-h-64 overflow-auto rounded-2xl border border-border-subtle bg-white text-sm shadow-xl"
                        >
                            <div class="py-1">
                                <!-- Users -->
                                <template v-if="userAndTeamsSearchResult.users?.length">
                                    <div class="px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-text-subtle">
                                        {{ $t('Users') }}
                                    </div>
                                    <div class="divide-y divide-border-subtle">
                                        ...
                                        >
                                            <img
                                                ...
                                                class="h-8 w-8 rounded-full object-cover ring-1 ring-border-subtle"
                                            />
                                            <div class="flex flex-col min-w-0">
                                                <span class="truncate text-xs font-medium text-text">
                                                    {{ user.first_name }} {{ user.last_name }}
                                                </span>
                                                <span v-if="user.position" class="truncate text-[11px] text-text-subtle">
                                                    {{ user.position }}
                                                </span>
                                            </div>
                                        </div>
                                ...
                                <div
                                    v-if="!userAndTeamsSearchResult.users?.length && !userAndTeamsSearchResult.departments?.length"
                                    class="px-4 py-3 text-xs text-text-subtle"
                                >
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="inline-flex items-center gap-1 rounded-full bg-surface-sunken px-2 py-0.5 text-[10px] text-text-subtle">
                            ...
                            <div
                                class="mt-1 flex flex-wrap gap-1.5 min-h-[1.75rem] rounded-xl bg-white/70 px-2 py-1 border border-dashed border-border-subtle"
                            >
                                <span
                                    v-for="user in selectedUsers"
                                    :key="user.id"
                                    class="inline-flex items-center gap-1 rounded-full bg-accent-50 text-accent-700 px-2 py-0.5 text-xs shadow-xs border border-accent-200"
                                >
                        ...
                        <div class="space-y-2">
                            ...
                            >
                                <span
                                    ...
                                    :key="department.id"
                                    class="inline-flex items-center gap-1 rounded-full bg-success-surface text-success px-2 py-0.5 text-xs shadow-xs border border-success-border"
                                >
                                    <span class="truncate max-w-[140px]">
                                        {{ department.name }}
                                    </span>
                ...
                <p v-else class="text-xs text-text-subtle flex items-center gap-1.5">
                    <span class="inline-flex h-4 w-4 items-center justify-center rounded-full bg-surface-sunken text-[10px] text-text-subtle">
                        i
                    </span>
