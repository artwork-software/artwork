    ...
    <app-layout :title="$t('My account')">
        <div>
            <div class="max-w-screen-lg py-4 pl-20 pr-4">
                <div v-if="$page.props.jetstream.canUpdateProfileInformation">
                    <form @submit.prevent="updateProfileInformation">
                        <div>
                            <div>
                                <h2 class="font-lexend font-black text-[clamp(24px,3vw,30px)]/[34px] text-text my-2">{{ $t('My account') }}</h2>
                                        ...
                                        <div class="sm:col-span-3 flex items-end">
                                            ...
                                            <div class="mt-1 ml-5 flex-grow relative">
                                                <input id="first_name" v-model="userForm.first_name" type="text"
                                                    class="peer pl-0 h-16 w-full focus:border-t-transparent focus:border-black focus:ring-black border-l-0 border-t-0 border-r-0 border-b-2 border-border text-xl font-bold text-text placeholder-text-subtle placeholder-transparent"
                                                    placeholder="placeholder"/>
                                                <label for="first_name"
                                                    class="absolute left-0 -top-5 text-text-muted text-sm -top-3.5 transition-all text-text-subtle peer-placeholder-shown:text-base peer-placeholder-shown:text-text-subtle peer-placeholder-shown:top-2 peer-focus:-top-3.5 peer-focus:text-sm ">{{$t('First name')}}</label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="sm:col-span-3 flex items-end">
                                            <div class="relative mt-1 w-full">
                                                <input id="last_name" v-model="userForm.last_name" type="text"
                                                   ...
                                                   border-b-2 border-border text-xl font-bold text-text placeholder-text-subtle placeholder-transparent"
                                                    placeholder="placeholder"/>
                                                <label for="last_name"
                                                    class="absolute left-0 -top-5 text-text-muted text-sm -top-3.5 transition-all subpixel-antialiased
                                                   text-text-subtle peer-placeholder-shown:text-base peer-placeholder-shown:text-text-subtle peer-placeholder-shown:top-2 peer-focus:-top-3.5 peer-focus:text-sm ">{{ $t('Last name')}}</label>
                                            </div>
                                        </div>

                                    </div>
                                    <div v-if="hasNameError" class="text-danger mt-1">{{ nameError }}</div>
                                    ...
                                    <div class="sm:col-span-3">
                                        <div class="mt-1">
                                            <input type="text" v-model="userForm.position" :placeholder="$t('Position')"
                                                class="text-text focus:border-text-subtle focus:border-1 border-2 w-full font-semibold border-border"/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="sm:col-span-3">
                                        <div class="mt-1 relative">
                                            <input type="email" v-model="userForm.email" :placeholder="$t('E-mail address')"
                                                :class="[email_validation_classes,'text-text border-2 w-full font-semibold focus:border-text-subtle focus:border-1']"/>

                                            <div v-if="!email_validation.email"
                                                class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                <IconCheck class="h-5 w-5 text-success" aria-hidden="true"/>
                                            </div>
                                            <div v-if="email_validation.email && email_validation.email.length > 0"
                                                class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                <IconX class="h-5 w-5 text-danger" aria-hidden="true"/>
                                            </div>
                                        </div>
                                    ...
                                    <div class="sm:col-span-3">
                                        <div class="mt-1">
                                            <input type="text" v-model="userForm.phone_number"
                                                ...
                                                class="text-text border-2 w-full focus:border-text-subtle focus:border-1 font-semibold border-border "/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="sm:col-span-6">
                                        <div class="mt-1">
                                            <textarea
                                                ...
                                                class="resize-none focus:border-text-subtle focus:border-1 border-2 w-full font-semibold border border-border "/>
                                        </div>
                                    </div>
                                    <div class="sm:col-span-6">
                                        <div class="sm:col-span-6 ml-3 flex inline-flex">
                                            <span v-if="userForm.departments.length === 0"
                                                class="text-text-subtle my-auto -ml-3">{{ $t('Not in any team')}}</span>
                ...
                <div v-if="$page.props.jetstream.canUpdatePassword">
                            ...
                            <div class="mt-1 relative rounded-md ">
                                <input
                                    ...
                                    :class="[passwordForm.hasErrors && passwordForm.errors.current_password
                                            ...
                                            ? 'border-success' : '',
                                    'placeholder-text-subtle border-border-subtle focus:border-text-subtle focus:border-1 border-2 block w-full']"/>
                        ...
                        <div class="w-full md:w-1/2">
                            <div class="mt-1 relative rounded-md ">
                                <input
                                    ...
                                    :class="[passwordForm.hasErrors ? 'border-danger' : 'border-border-subtle',
                                    'placeholder-text-subtle focus:border-text-subtle focus:border-1 border-2 block w-full']"/>
                    ...
                    <div class="mt-4">
                        <div class="flex items-center text-text-subtle w-full">
            ...
            <div class="flex ml-20 mt-12">
                <span v-if="$role('artwork admin')" @click="openDeleteUserModal()" class="text-sm/5 font-bold text-text-subtle cursor-pointer">{{$t('Delete account permanently')}}</span>
            </div>
        </div>
        ...
        <BaseModal @closed="closeChangePictureModal" v-if="showChangePictureModal" modal-image="/Svgs/Overlays/illu_project_history.svg" :show-image="false">
                <div class="mx-4">
                    ...
                    <span class="text-text-subtle my-auto">{{ $t('Select your profile picture here. It should not exceed the size of 3072 KB.')}} </span>
                    ...
                    <div class="flex justify-between mt-6">
                        ...
                        <div class="flex my-auto">
                            <span @click="closeDeleteUserModal()"
                                class="text-text-subtle text-sm/5 font-bold text-text-subtle cursor-pointer">{{$t('No, not really')}}</span>
                        </div>
                    </div>
                </div>
        </BaseModal>
