<template>
    <div class="mb-8 border-b border-dashed border-gray-400 pb-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-sm font-semibold text-gray-900">{{ $t('Revenue') }}</h3>
            <div v-if="canEdit">
                <SwitchDualLabel
...
const revenueTotal = ref(null);

watch(() => props.biData, (val) => {
    if (val) {
        currentMode.value = val.revenue_mode ?? 'total';
        revenueTotal.value = val.revenue_total;
    }
}, { immediate: true });
...
const onToggleChange = async (isPerEvent) => {
    ...
    try {
        await axios.put(route('projects.bi.switch-revenue-mode', props.projectId), { mode: newMode });
        emit('updated');
    } catch (error) {
        console.error('Error switching revenue mode', error);
    }
};

const saveTotal = async () => {
    try {
        await axios.put(route('projects.bi.update-data', props.projectId), { revenue_total: revenueTotal.value });
        emit('updated');
    } catch (error) {
        console.error('Error saving revenue total', error);
    }
};
</script>
