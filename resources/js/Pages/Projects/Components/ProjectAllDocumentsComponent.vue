...
const props = defineProps<{
    project: { id: number | string; project_files_tab?: ProjectFile[]; project_files_all?: ProjectFile[] }
...
/** Rollen aus globalProperties (falls vorhanden) */
...
async function validateTypeAndUpload(files: File[]) {
    ...
    for (const f of files) {
        try {
            await uploadDocumentToProject(f)
            uploadedCount.value++
        } catch {
            // Fehler wurden schon gesetzt
        }
    }
...
function openConfirmDeleteModal(file: ProjectFile) {
...
function formatDate(iso?: string) {
    if (!iso) return ''
    try {
        return new Date(iso).toLocaleDateString((page.props as any)?.locale ?? 'de-DE', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
        })
...
const { pdf: lightboxPdf, pages } = usePDF(lightboxSrc)
...
watch(pages, (n) => { if (n && currentPage.value > n) currentPage.value = n })
...
<template>
    <div class="my-6 space-y-4">
        ...
        <!-- Fehlermeldungen -->
        <div v-if="errorCount > 0" class="mb-2 print:hidden">
            <MultiAlertComponent :errors="documentForm.errors" :error-count="errorCount" />
        </div>

        <!-- Dropzone -->
        <div v-if="canEdit" class="relative print:hidden">
            <div
                role="button"
                tabindex="0"
