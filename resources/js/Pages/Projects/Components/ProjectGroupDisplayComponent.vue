<template>
    <div v-if="project.is_group && headerObject?.projectsOfGroup?.length > 0">
        ...
        <div v-if="headerObject?.projectsOfGroup?.length > 0" class="flex items-center gap-4 mt-3">
            <div v-for="(groupProject, index) in headerObject?.projectsOfGroup" class="group block shrink-0 bg-accent-600/80 w-fit rounded-full border border-accent-600/90 hover:bg-accent-600 duration-200 ease-in-out">
