        ...
        <div class="rounded-2xl border border-border-subtle bg-white shadow-sm">
            ...
            <header class="flex items-center justify-between gap-3 border-b border-border-subtle px-6 py-4">
                ...
                <div class="hidden sm:flex items-center gap-2">
                  <span class="inline-flex items-center rounded-full border border-border-subtle px-2 py-0.5 text-[11px] text-text-muted">
            ...
            <div class="px-2 pb-6 sm:px-6">
                <div v-if="newCommentList?.length > 0" class="relative">
                    <!-- Timeline line -->
                    <div class="absolute left-4 top-0 h-full w-px bg-border-subtle sm:left-5"></div>
                    ...
                    <ul class="space-y-4">
                      ...
                      "
                                            type="button"
                                            class="invisible group-hover:visible inline-flex rounded-full p-1 text-text-subtle transition hover:bg-danger-surface hover:text-danger focus-visible:outline-none"
                ...
                <div v-else class="rounded-xl border border-dashed border-border-subtle bg-surface-sunken p-10 text-center">
                    <div class="mx-auto max-w-md">
                        ...
                        <h3 class="text-sm font-semibold text-text">{{ $t('No comments yet') }}</h3>
...
const initialComments = props.project?.comments_all ?? [];
...
async function fetchComments() {
    ...
    try {
        const { data } = await axios.get(
            route('projects.tabs.all-comments', { project: projectId })
        );
