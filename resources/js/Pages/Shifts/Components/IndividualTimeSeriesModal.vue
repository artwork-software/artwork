        ...
        <div class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <div class="space-y-5">
                    ...
                    <!-- Times -->
                    <div class="space-y-2 rounded-xl border border-border-subtle bg-white/80 p-4 shadow-sm">
                        <div class="flex items-center justify-between gap-2">
                            <div class="flex items-center gap-2 text-xs font-medium text-text-muted">
                                <PropertyIcon name="IconClock" class="h-4 w-4" />
                                <span>{{ $t('Time') }}</span>
                            </div>
                            <label class="inline-flex cursor-pointer items-center gap-2 text-xs text-text-muted">
                                <input
                                    v-model="form.full_day"
                                    type="checkbox"
                                    class="h-3.5 w-3.5 rounded border-border text-accent-600 focus:ring-accent-600"
                                />
                        ...
                        <div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <div class="space-y-1.5">
                                <label class="text-[11px] font-medium uppercase tracking-wide text-text-subtle">
                                    {{ $t('Start') }}
                                </label>
                                <BaseInput
                        ...
                        <!-- Break Minutes -->
                        <div v-if="!form.full_day" class="space-y-1.5 pt-2 border-t border-border-subtle">
                            <BaseInput
                            ...
                            />
                            <p class="text-[11px] text-text-subtle leading-snug">
                                {{ infoText }}
                            </p>
                        </div>
                    </div>
                    ...
                    <!-- Recurrence -->
                    <div class="space-y-3 rounded-xl border border-border-subtle bg-white/80 p-4 shadow-sm">
                        <div class="flex items-center gap-2 text-xs font-medium text-text-muted">
                            <PropertyIcon name="IconRepeat" class="h-4 w-4" />
                        ...
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <select
                                    ...
                                    class="w-full rounded-lg border border-border-subtle bg-white px-3 py-3 shadow-sm text-sm outline-none focus:border-accent-600 focus:ring-1 focus:ring-accent-600"
                                >
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <button
                                    type="button"
                                    class="text-[11px] font-medium text-accent-600 hover:text-accent-700"
                            ...
                            <div class="grid grid-cols-7 gap-1.5">
                                <button
                                    ...
                                    :key="day.value"
                                    type="button"
                                    class="flex h-8 items-center justify-center rounded-lg border text-xs font-medium transition hover:border-accent-200 hover:text-accent-600"
                                    :class="form.weekdays.includes(day.value) ? 'border-accent-600 bg-accent-50 text-accent-700 shadow-[0_0_0_1px_rgba(79,70,229,0.25)]'
                ...
                <div class="space-y-5">
                    ...
                    <div class="space-y-2 rounded-xl border border-border-subtle bg-white/80 p-4 shadow-sm">
                        ...
                        <div class="mt-3 max-h-56 space-y-1 overflow-y-auto rounded-lg bg-surface-sunken/60 p-2">
                            ...
                            >
                                <span class="h-2 w-2 animate-pulse rounded-full bg-accent-600"></span>
                                <span>{{ $t('Searching...') }}</span>
                            </div>
                            <div
                                v-else-if="searchResults.length === 0 && search.length > 0"
                                class="rounded-md bg-white/80 px-3 py-2 text-xs text-text-subtle"
                            >
                                ...
                                >
                                    <div class="flex items-center gap-2">
                                        <img
                                            ...
                                            class="h-6 w-6 rounded-full object-cover ring-1 ring-border-subtle"
                                            alt=""
                                        />
                                        <div>
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        <span
                                            ...
                                            class="inline-flex items-center gap-1 rounded-full bg-success-surface px-2 py-0.5 text-[10px] font-medium text-success"
                                        >
                                        ...
                                        <span
                                            v-else
                                            class="inline-flex items-center gap-1 rounded-full bg-accent-50 px-2 py-0.5 text-[10px] font-medium text-accent-700 group-hover:bg-accent-100"
                                        >
                    ...
                    <div class="space-y-2 rounded-xl border border-border-subtle bg-white/80 p-4 shadow-sm">
                        ...
                        >
                            <div
                                ...
                                class="inline-flex items-center rounded-full bg-surface-sunken text-[11px] text-text-muted shadow-sm"
                            >
                                <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-border-subtle text-[10px] font-semibold text-text-muted">
                                ...
                                <div class="inline-flex items-center gap-1.5 py-1 px-2">
                                    ...
                                    <span class="rounded-full bg-white/80 px-1.5 py-0.5 text-[9px] uppercase tracking-wide text-text-subtle">
            ...
            <!-- Footer -->
            <div class="grid grid-cols-2 border-t border-border-subtle pt-4 gap-3">
...
// manuelle Werte nie still überschreiben); nicht bei "ganztägig".
const startTimeRef = computed(() => form.start_time);
const endTimeRef = computed(() => form.end_time);
const { infoText } = useLegalBreak(startTimeRef, endTimeRef);
const autoBreak = useAutoBreak(
    startTimeRef,
    endTimeRef,
    computed({
        get: () => form.break_minutes,
        set: (v) => { form.break_minutes = (v ?? 0) as number; },
    }),
    { enabled: computed(() => !form.full_day) }
);
...
async function loadSeries(uuid: string) {
    try {
        ...
        autoBreak.applyExternalValue(data.break_minutes ?? 0);
