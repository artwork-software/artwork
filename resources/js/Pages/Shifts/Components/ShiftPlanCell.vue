<template>
    ...
    >
        ...
        <div v-if="openViolations.length || resolvedViolations.length || unavailableAssignmentConflict" class="absolute top-0.5 right-0.5 flex items-center gap-0.5">
            ...
            >
                <!-- Ohne Regel (manuell "Sonstiges"): Warnfarbe aus dem Design-Token statt Regelfarbe -->
                <svg class="h-3.5 w-3.5" :class="openViolationColorClass" :style="openViolationColorStyle" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                </svg>
                <span
                    v-if="openViolations.length > 1"
                    class="text-[9px] font-semibold leading-none rounded-full bg-white/90 px-1 py-px"
                    :class="openViolationColorClass"
                    :style="openViolationColorStyle"
                >
...
/** Farbe des ersten offenen Verstoßes: Regelfarbe, ohne Regel (manuell "Sonstiges") text-warning */
const openViolationColorStyle = computed(() => {
    const rule = openViolations.value[0]?.shift_rule
    return rule ? { color: rule.warning_color || '#ff0000' } : null
})
const openViolationColorClass = computed(() => (openViolations.value[0]?.shift_rule ? '' : 'text-warning'))
