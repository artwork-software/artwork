    ...
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            <tr v-for="row in rows" :key="row.id">
                ...
                <td class="py-2 px-2">
                    <span :class="!granted && isExpired(row.deadline) ? 'text-red-600' : ''">
                        {{ formatDate(granted ? row.granted_date : row.deadline) }}
                    </span>
                    <span v-if="!granted && isExpired(row.deadline)" class="text-xs text-red-600">
                        ({{ $t('Deadline expired') }})
                    </span>
                </td>
                <td class="py-2 pl-2 text-gray-500">{{ row.reason }}</td>
            </tr>
            <tr v-if="!rows || !rows.length">
                <td colspan="3" class="py-4 text-center text-gray-400">{{ $t(empty) }}</td>
            </tr>
        </tbody>
    </table>
</template>
