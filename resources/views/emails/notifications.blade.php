                    ...
                    <div class="notification-text">
                        ...
                        <p>
                            @if($body['body']['type'] === 'ROOM_REQUEST')
                                ...
                                {{ $body['body']['event']['eventName'] }} | @if(!empty($body['body']['event']['project_id'])) {{ \App\Models\Project::find($body['body']['event']['project_id'])->first()->name }} | @endif
                            ...
                            @if($body['body']['type'] === 'NOTIFICATION_UPSERT_ROOM_REQUEST')
                                {{ \App\Models\EventType::find($body['body']['event']['event_type_id'])->first()->name }},
                                {{ $body['body']['event']['eventName'] }} | @if(!empty($body['body']['event']['project_id'])) {{ \App\Models\Project::find($body['body']['event']['project_id'])->first()->name }} | @endif
                                {{ date('d.m.Y H:i', strtotime($body['body']['event']['start_time'])) }} -  {{ date('d.m.Y H:i', strtotime($body['body']['event']['end_time'])) }}
                            @endif
                            @if($body['body']['type'] === 'NOTIFICATION_CONFLICT')
                                {{ \App\Models\Room::find($body['body']['conflict']['event']['room_id'])->first()->name }} | {{ \App\Models\EventType::find($body['body']['conflict']['event']['event_type_id'])->first()->name }},
                                {{ $body['body']['conflict']['event']['eventName'] }} | @if(!empty($body['body']['conflict']['event']['project_id'])) {{ \App\Models\Project::find($body['body']['conflict']['event']['project_id'])->first()->name }} | @endif
